...
export default function HomeHero({ module }: HomeHeroProps) {
    ...
    const imageUrl = imageBuilder && 'width' in imageBuilder
        ? (imageBuilder as { width: (w: number) => { quality: (q: number) => { url: () => string } } }).width(1200).quality(90).url()
            ...
            <Container maxWidth="lg">
                <Box
                    sx={{
                        ...
                        color: module.emphasis === 'brand' ? 'var(--color-on-primary)' : 'var(--color-on-surface)',
                        borderRadius: 2,
                        overflow: 'hidden',
                    }}
                >
